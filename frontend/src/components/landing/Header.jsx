...
const Header = () => {
  ...
  const user = {name: 'Hayou', email: 'hayouayana@gmail.com'}
  const logout = () => {};
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            { isAuthenticated ? (
              <ProfileDropdown
                ...
                onLogout={logout}
              />
            ) : (<><Link to='/login' className="text-black hover:text-gray-600 font-medium transition-colors duration-200">Login</Link>
          ...
          <div className="lg:hidden">
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="p-2 rounded-lg text-gray-600 hover:text-gray-900 transition-colors duration-200">{isMenuOpen ? ( <X className="w-6 h-6"/>) : (<Menu className="w-6 h-6" />)} </button>
          </div>
        </div>
      </div>

      {isMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 bg-white border-b border-gray-200 shadow-lg">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            {isAuthenticated ? (
              ...
              <>
                <Link to='/login' className="block px-4 py-3 text-gray-600 hover:text-gray-900 hover:gb-gray-50 font-medium transition-colors duration-200">Login</Link>
